...
def original_version(trial_list):
    ...
    for x in trial_list:
        try:
            ...
            for l in locations:
                l.update(
                    {"nctid": x["protocolSection"]["identificationModule"]["nctId"]}
                )
...
def original_version_if(trial_list):
    ...
    for x in trial_list:
        if (
            "contactsLocationsModule" in x["protocolSection"]
            and "locations" in x["protocolSection"]["contactsLocationsModule"]
        ):
            locations = x["protocolSection"]["contactsLocationsModule"]["locations"]
            for l in locations:
                l.update(
...
def chatgpt_version(trial_list):
    locations_list = [
        ...
        if "contactsLocationsModule" in item["protocolSection"]
        and "locations" in item["protocolSection"]["contactsLocationsModule"]
...
print(
    f"Original: {timeit.timeit('original_version(trial_list)', setup=setup, number=10)}"
)

setup = "from __main__ import original_version_if, trial_list"
print(
    f"Original (if): {timeit.timeit('original_version_if(trial_list)', setup=setup, number=10)}"
)

setup = "from __main__ import chatgpt_version, trial_list"
print(
    f"ChatGPT: {timeit.timeit('chatgpt_version(trial_list)', setup=setup, number=10)}"
)

setup = "from __main__ import tom_version, trial_list"
print(
    f"TomW (try/except): {timeit.timeit('tom_version(trial_list)', setup=setup, number=10)}"
)

setup = "from __main__ import tom_version2, trial_list"
print(f"TomW (if): {timeit.timeit('tom_version2(trial_list)', setup=setup, number=10)}")
